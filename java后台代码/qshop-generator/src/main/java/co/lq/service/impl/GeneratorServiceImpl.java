...
import co.lq.domain.vo.TableInfo;
import co.lq.exception.BadRequestException;
import co.lq.repository.ColumnInfoRepository;
import co.lq.service.GeneratorService;
import co.lq.utils.FileUtil;
import co.lq.utils.GenUtil;
    ...
    public void sync(List<ColumnInfo> columnInfos, List<ColumnInfo> columnInfoList) {
        ...
        for (ColumnInfo columnInfo : columnInfoList) {
            ...
            List<ColumnInfo> columns = new ArrayList<ColumnInfo>(
                    columnInfos.stream().filter(c -> c.getColumnName().equals(columnInfo.getColumnName())).collect(
                            Collectors.toList()));
            // 如果能找到，就修改部分可能被字段
        ...
        for (ColumnInfo columnInfo : columnInfos) {
            // 根据字段名称查找
            List<ColumnInfo> columns = new ArrayList<ColumnInfo>(
                    columnInfoList.stream().filter(c -> c.getColumnName().equals(columnInfo.getColumnName())).collect(
                            Collectors.toList()));

...
public class TreeUtil {
     ...
     * @return
     */
    public static List<CatalogDTO> listCatalogTreeConverter(List<CatalogDTO> list, int parentId) {
        List<CatalogDTO> returnList = new ArrayList<>();

        for (CatalogDTO res : list) {
            //判断对象是否为根节点

            if (res.getPid() == parentId) {
                //该节点为根节点,开始递归

                recursionFnCatalog(list, res); //通过递归为节点设置childList
     ...
     * @param t
     */
    public static void recursionFnCatalog(List<CatalogDTO> list, CatalogDTO t) {
        //只能获取当前t节点的子节点集,并不是所有子节点集
        List<CatalogDTO> childsList = getChildListCatalog(list, t);
        //设置他的子集对象集
        t.setChildren(childsList);

        //迭代子集对象集
        for (CatalogDTO nextChild : childsList) { //遍历完,则退出递归

            //判断子集对象是否还有子节点
            if (!CollectionUtils.isEmpty(childsList)) {
                //有下一个子节点,继续递归
                recursionFnCatalog(list, nextChild);
            }
        }
    }
     ...
     * @return
     */
    public static List<CatalogDTO> getChildListCatalog(List<CatalogDTO> list, CatalogDTO t) {
        List<CatalogDTO> childsList = new ArrayList<>();
        //遍历集合元素,如果元素的Parentid==指定元素的id,则说明是该元素的子节点
        for (CatalogDTO t1 : list) {
            if (t1.getPid().equals(t.getId())) {
                childsList.add(t1);
            }
        }

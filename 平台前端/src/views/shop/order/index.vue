<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="data" size="small" style="width: 100%;">
      ...
      <el-table-column prop="cartInfoList" width="300" label="商品信息">
        <template slot-scope="scope">
          <div v-for="(item,index) in scope.row.cartInfoList" v-if="item.cartInfoMap.productInfo.attrInfo">
            ...
            <span>{{ item.cartInfoMap.productInfo.productName }}&nbsp;{{ item.cartInfoMap.productInfo.attrInfo.suk }}</span>
          ...
          <div v-else>
            ...
            ></span>
            <span>{{ item.cartInfoMap.productInfo.productName }}</span>
            <span> | ￥{{ item.cartInfoMap.truePrice }}×{{ item.cartInfoMap.cartNum }}</span>
          </div>
        </template>
      </el-table-column>
      ...
      <el-table-column v-if="checkPermission(['admin','STOREORDER_ALL','STOREORDER_EDIT','STOREORDER_DELETE'])" label="操作" width="200" align="center" fixed="right">
        <template slot-scope="scope">
          <el-button
            v-permission="['admin','STOREORDER_ALL','STOREORDER_EDIT']"
            size="mini"
            type="primary"
          ...
          <!-- <el-dropdown size="mini" split-button type="primary" trigger="click">
            ...
            <el-dropdown-menu slot="dropdown">
              ...
              <el-dropdown-item v-if="scope.row._status == 1">
                <el-popover
                  :ref="scope.row.id"
                  v-permission="['admin','STOREORDER_ALL','STOREORDER_DELETE']"
                  placement="top"
                  width="180"
                >
                  <p>确定删除本条数据吗？</p>
...
import { del } from '@/api/storeOrder'
...
export default {
  components: { eForm, eDetail, eRefund, editOrder, eRemark, parseTime },
  mixins: [initData],
  data() {
    return {

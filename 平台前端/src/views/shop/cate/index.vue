<template>
  <div class="app-container">
    ...
    <!--表单组件-->
    <el-dialog append-to-body :close-on-click-modal="false" :before-close="crud.cancelCU" :visible.sync="crud.status.cu > 0" :title="crud.status.title" width="560px">
      <el-form ref="form" :model="form" :rules="rules" size="small" label-width="100px">
        ...
        <el-form-item style="margin-bottom: 0;" label="上级类目" prop="pid">
          <treeselect v-model="form.catalogId" :options="catalogs" style="width: 370px;" placeholder="选择上级类目" />
        </el-form-item>
      </el-form>
...
// crud交由presenter持有
const defaultCrud = CRUD({ title: '分类', url: 'api/category', crudMethod: { ...crudDept, crudCatalog }})
const defaultForm = { id: null, cateName: null, pid: 0, catalogId: 0, isShow: 1 }
export default {
  name: 'Dept',
  ...
  data() {
    return {
      ...
      permission: {
        add: ['admin', 'STORECATEGORY_CREATE'],
        edit: ['admin', 'STORECATEGORY_EDIT'],
        del: ['admin', 'STORECATEGORY_DELETE']
      },
      enabledTypeOptions: [
    ...
    [CRUD.HOOK.afterToCU](crud, form) {
      ...
      crudDept.getCates({ isShow: true }).then(res => {
        ...
        if (dept.children.length>0){
          dept.children.forEach((item, index) => {
              dept.children[index].children = null;
          });
        }
      ...
      // 获取所有类目
      crudCatalog.getCatalogs({ isShow: true }).then(res => {
        this.catalogs = []
        const catalogs = { id: 0, label: '顶级类目', children: [] }
        catalogs.children = res.content

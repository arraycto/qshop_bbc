...
<script>
import crudUserRecharge from '@/api/userRecharge'
...
// crud交由presenter持有
const defaultCrud = CRUD({ title: '充值管理', url: 'api/userRecharge', sort: 'id,desc', crudMethod: { ...crudUserRecharge }, optShow: { add: false, edit: false, del: true, download: true}})
...
export default {
  ...
  data() {
    return {
      permission: {
        add: ['admin', 'userRecharge:add'],
        edit: ['admin', 'userRecharge:edit'],
        del: ['admin', 'userRecharge:del']
      },
      rules: {
      },
      queryTypeOptions: [

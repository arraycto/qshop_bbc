  ...
  <div class="app-container">
    ...
    <div class="head-container">
      ...
      <el-dialog :close-on-click-modal="false" :before-close="crud.cancelCU" :visible.sync="crud.status.cu > 0" :title="crud.status.title" width="560px">
        <el-form ref="form" :model="form" :rules="rules" size="small" label-width="100px">
          ...
          <el-form-item label="contentId">
            <el-input v-model="form.contentId" style="width: 370px;" />
          </el-form-item>
      ...
      <el-table ref="table" v-loading="crud.loading" :data="crud.data" size="small" style="width: 100%;" @selection-change="crud.selectionChangeHandler">
        ...
        <el-table-column v-if="columns.visible('phone')" prop="phone" label="phone" />
        <el-table-column v-if="columns.visible('status')" prop="status" label="status" />
        <el-table-column v-if="columns.visible('content')" prop="content" label="content" />
        <el-table-column v-if="columns.visible('result')" prop="result" label="result" />
        <el-table-column v-if="columns.visible('contentId')" prop="contentId" label="contentId" />
        ...
        <el-table-column v-permission="['admin','smsContentMsg:edit','smsContentMsg:del']" label="操作" width="150px" align="center">
          <template slot-scope="scope">
            <udOperation
              :data="scope.row"
...
<script>
import crudSmsContentMsg from '@/api/smsContentMsg'
...
// crud交由presenter持有
const defaultCrud = CRUD({ title: '短信发送记录', url: 'api/smsContentMsg', sort: 'id,desc', crudMethod: { ...crudSmsContentMsg }})
const defaultForm = { id: null, phone: null, status: null, content: null, result: null, contentId: null, type: null, storeId: null, addTime: null, modifyTime: null, deleted: null }
export default {
  name: 'SmsContentMsg',
  ...
  data() {
    return {
      permission: {
        add: ['admin', 'smsContentMsg:add'],
        edit: ['admin', 'smsContentMsg:edit'],
        del: ['admin', 'smsContentMsg:del']
      },
      rules: {
        addTime: [

<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="data" size="small" style="width: 100%;">
      ...
      <!-- <el-table-column v-if="checkPermission(['admin','STORECOMBINATION_ALL','STORECOMBINATION_EDIT','STORECOMBINATION_DELETE'])" label="操作" width="150px" align="center" fixed="right">
        <template slot-scope="scope">
          <el-button v-permission="['admin','STORECOMBINATION_ALL','STORECOMBINATION_EDIT']" size="mini" type="primary" icon="el-icon-edit" @click="edit(scope.row)" />
          <el-popover
            :ref="scope.row.id"
            v-permission="['admin','STORECOMBINATION_ALL','STORECOMBINATION_DELETE']"
            placement="top"
            width="180"
          >
            <p>确定删除本条数据吗？</p>
...
import { del, onsale, onVerify } from '@/api/storeCombination'
  ...
  methods: {
    ...
    onVerify(id, status) {
      this.$confirm(`确定进行[${status ? '不通过' : '通过'}]操作?`, '提示', {
        ...
        .then(() => {
          onVerify({ status: status, combinationId: id }).then(({ data }) => {
            this.$message({
              message: '操作成功',
              type: 'success',
              duration: 1000,

    ...
    <div class="head-container">
      ...
      <el-dialog :close-on-click-modal="false" :before-close="crud.cancelCU" :visible.sync="crud.status.cu > 0" :title="crud.status.title" width="560px">
        <el-form ref="form" :model="form" :rules="rules" size="small" label-width="100px">
          ...
          <el-form-item label="combinationId">
      ...
      <el-table ref="table" v-loading="crud.loading" :data="crud.data" size="small" style="width: 100%;" @selection-change="crud.selectionChangeHandler">
        ...
        <el-table-column v-if="columns.visible('combinationId')" prop="combinationId" label="combinationId" />
        ...
        <el-table-column v-permission="['admin','combinationVertifyRecord:edit','combinationVertifyRecord:del']" label="操作" width="150px" align="center">
          <template slot-scope="scope">
            <udOperation
              :data="scope.row"
...
<script>
import crudCombinationVertifyRecord from '@/api/combinationVertifyRecord'
...
// crud交由presenter持有
const defaultCrud = CRUD({ title: '拼团审核纪录', url: 'api/combinationVertifyRecord', sort: 'id,desc', crudMethod: { ...crudCombinationVertifyRecord }})
const defaultForm = { id: null, combinationId: null, createTime: null, vertifyMan: null, status: null, detail: null, storeId: null }
export default {
  name: 'CombinationVertifyRecord',
  components: { pagination, crudOperation, rrOperation, udOperation },
  mixins: [presenter(defaultCrud), header(), form(defaultForm), crud()],
  data() {
    return {
      permission: {
        add: ['admin', 'combinationVertifyRecord:add'],
        edit: ['admin', 'combinationVertifyRecord:edit'],
        del: ['admin', 'combinationVertifyRecord:del']
      },
      rules: {
      }    }
  },

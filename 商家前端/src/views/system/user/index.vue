          ...
          <el-form ref="form" :inline="true" :model="form" :rules="rules" size="small" label-width="66px">
            ...
            <el-form-item label="状态">
              <el-radio-group v-model="form.enabled" :disabled="form.id === user.id">
                ...
                <el-radio v-model="form.enabled" :label="0" style="width: 110px;">禁用</el-radio>
              </el-radio-group>
            </el-form-item>
        ...
        <el-table ref="table" v-loading="crud.loading" :data="crud.data" style="width: 100%;" @selection-change="crud.selectionChangeHandler">
          ...
          <el-table-column v-if="columns.visible('dept')" :show-overflow-tooltip="true" width="110" prop="dept" label="部门 / 岗位">
            <template slot-scope="scope">
              <div>{{ scope.row.sellerDept.name }} / {{ scope.row.sellerJob.name }}</div>
            </template>
          </el-table-column>
...
export default {
  ...
  methods: {
    ...
    [CRUD.HOOK.beforeToEdit](crud, form) {
      this.getJobs(this.form.sellerDept.id)
      userRoles = []
      const roles = []
    ...
    changeEnabled(data, val) {
      ...
      this.$confirm('此操作将 "' + opLabel + '" ' + data.username + ', 是否继续？', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        crudUser.edit(data).then(res => {
          this.crud.notify(opLabel + '成功', CRUD.NOTIFICATION_TYPE.SUCCESS)
        }).catch(() => {

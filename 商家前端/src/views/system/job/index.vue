<template>
  <div class="app-container">
    ...
    <el-table ref="table" v-loading="crud.loading" :data="crud.data" style="width: 100%;" @selection-change="crud.selectionChangeHandler">
      ...
      <el-table-column v-if="columns.visible('sellerDept')" prop="sellerDept" label="所属部门">
        <template slot-scope="scope">
          <div>{{ scope.row.deptSuperiorName ? scope.row.deptSuperiorName + ' / ' : '' }}{{ scope.row.sellerDept.name }}</div>
        </template>
      </el-table-column>
    ...
    changeEnabled(data, val) {
      ...
      this.$confirm('此操作将 "' + this.dict.job_status[value].label + '" ' + data.name + '岗位, 是否继续？', '提示', {
        ...
        crud.crudMethod.edit(data).then(() => {
          crud.notify(this.dict.job_status[value].label + '成功', 'success')
        }).catch(err => {

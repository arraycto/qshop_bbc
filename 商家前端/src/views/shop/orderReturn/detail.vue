    ...
    <el-card shadow="never" class="standard-margin">
      ...
      <div class="form-container-border">
        ...
        <el-row>
          ...
          <el-col class="form-border font-small" :span="18">{{ parseTime(orderReturnApply.addTime) }}</el-col>
        </el-row>
        <el-row>
      ...
      <div class="form-container-border" v-show="orderReturnApply.status!==0">
        ...
        <el-row>
          ...
          <el-col class="form-border font-small" :span="18">{{ parseTime(orderReturnApply.modifyTime) }}</el-col>
        </el-row>
        <el-row>
      ...
      <div class="form-container-border" v-show="orderReturnApply.status===2">
        ...
        <el-row>
          ...
          <el-col class="form-border font-small" :span="18">{{ parseTime(orderReturnApply.receiveTime) }}</el-col>
        </el-row>
        <el-row>
...
<script>
  import {getReturnApplyInfo as getReturnApplyList,edit} from '@/api/orderReturnApply';
  import {getCompanyList} from '@/api/companyAddress';
  ...
  export default {
    ...
    computed: {
      totalAmount() {
        if (this.orderReturnApply != null) {
          return this.orderReturnApply.returnAmount;
        } else {
          return 0;
        }
      },
      currentAddress() {
        console.log("currentAddress()", this.updateStatusParam.companyAddressId);
    ...
    methods: {
      ...
      getDetail() {
        getReturnApplyList(this.id).then(response => {
          console.log("getDetail")
          this.orderReturnApply = response;
          this.productList = [];
          this.productList.push(this.orderReturnApply.orderReturnApplyItem);
      ...
      getCompanyAddressList() {
        if (this.addressId!=undefined && this.addressId !=null){
          this.listQuery.id = this.addressId;
        }
        getCompanyList(this.listQuery).then(response => {
          this.companyAddressList = response.content;
          console.log("getCompanyAddressList()", )
          for (let i = 0; i < this.companyAddressList.length; i++) {
            if (this.companyAddressList[i].receiveStatus === 1&&this.orderReturnApply.status!=3) {
      ...
      handleUpdateStatus(status){
        ...
        this.$confirm('是否要进行此操作?', '提示', {
          ...
          edit({id: this.id,status: this.updateStatusParam.status}).then(response=>{
            this.$message({
              type: 'success',
              message: '操作成功!',
              duration:1000

<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="data" size="small" style="width: 100%;">
      ...
      <el-table-column v-if="checkPermission(['admin','STOREPRODUCT_ALL','STOREPRODUCT_EDIT','STOREPRODUCT_DELETE'])" label="操作" width="150px" align="center">
        <template slot-scope="scope">
          <el-button v-permission="['admin','STOREPRODUCT_ALL','STOREPRODUCT_EDIT']" size="mini" type="primary" icon="el-icon-edit" @click="edit(scope.row)" />
          <el-popover
            :ref="scope.row.id"
            v-permission="['admin','STOREPRODUCT_ALL','STOREPRODUCT_DELETE']"
            placement="top"
            width="180"
          >
            <p>确定恢复本条数据吗？</p>
...
import { recovery, onsale } from '@/api/storeProduct'
...
export default {
  ...
  methods: {
    ...
    add() {
      ...
      this.$refs.form.getPlatformCates()
      this.$refs.form.getPlatformBrands()
    },
    edit(data) {
      this.isAdd = false
      const _this = this.$refs.form
      _this.getCates()
      _this.getPlatformCates()
      _this.getPlatformBrands()
      _this.form = {
        id: data.id,
        merId: data.merId,
        image: data.image,
